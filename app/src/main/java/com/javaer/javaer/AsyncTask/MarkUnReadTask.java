...
public class MarkUnReadTask extends AsyncTask<List<String>, Void, Void>{

    ClassLoader classLoader;

    public MarkUnReadTask(ClassLoader classLoader) {
    ...
    protected Void doInBackground(List<String>[] lists) {
        ...
        try {
            for (int i = 0; i < wxids.size(); i++){
                XposedHelpers.callMethod(asInstance, Version.Message_Class_Method_2, wxids.get(i));
                Thread.sleep(1);
            }
        }catch (Exception e){
            e.printStackTrace();
